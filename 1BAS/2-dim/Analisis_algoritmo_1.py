...
for i in range(0,40):
	path = 'Resultado_BAS_'+str(i+1)+".txt"
...
Peor_solucion  = np.genfromtxt('Resultado_BAS_'+str(indice_peor + 1)+".txt")
Mejor_solucion = np.genfromtxt('Resultado_BAS_'+str(indice_mejor + 1)+".txt")
...
print("La mejor solución usando el algoritmo BAS: ",np.min(F0))
print("Se encuentran en las coordenadas: ", Mejor_solucion[-1,:][1:dim+1] ,"\n")

print("La peor solución usando el algoritmo BAS:",  np.max(F0))
...
plt.ylim(-8,8)
plt.xlim(-8,8)
plt.title(r'Algoritmo BAS 2D: espacio de solución')
plt.xlabel(r'$x_{1}$')
plt.ylabel(r'$x_{2}$')
plt.tight_layout()
plt.savefig('1BAS.png')
plt.clf()
...
plt.ylabel("FO")
plt.tight_layout()
plt.savefig("Grafico_Convergencia_BAS_2D.png")
